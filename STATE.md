...
- **Refinements (2026-06-06, post-fix) ✓:** (1) type-mismatch fix — reporting_guideline-vs-journal-article disambiguation (`REPORTING_GUIDELINE_GUIDE`), eliminated all type mismatches; (2) console **redeployed** (rev console-00005-xn4) with qa_audit UI; (3) **run-batch + sync-to-compendium jobs rebuilt** from a dedicated batch image (`Dockerfile.batch` + `scripts/deploy_batch_job.sh`), dry-run smoke-tested; (4) **ADK gold eval** run with authored rubrics → 19/20 pass, rubric 20/20 (see analysis §9). Final clean re-run: review_queue=49, source-accuracy 38/7/4, 0 type/methodology errors.
- Infra hardening: `FIRESTORE_COLLECTION_PREFIX` (isolates eval from prod Firestore), `GRPC_ENABLE_FORK_SUPPORT=1` (stops gRPC-after-fork SIGABRTs), qc_panel score coercion (KeyError fix). 276 tests green.
- Known residuals (documented, not blocking): interactive `adk web` path emits out-of-enum resource_type_code / omits editorial_description on some cases (deterministic production path unaffected); eval rubric judge scores all-pass (read as "no violations").
