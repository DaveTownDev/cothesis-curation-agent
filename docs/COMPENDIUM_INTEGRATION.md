...
| HITL Console | Publish gate: reviewer approves → `editorial_status: published` → **auto-POST to Compendium** |
...
flowchart TD
    ...
    J --> K[POST /api/import/json]
    K --> L[Compendium import pipeline]
...
|---|---|---|
| **Single approve** (review detail) | `approveItem()` → `trySyncAfterApprove()` | Fire-and-forget POST immediately after Firestore commit |
| **Bulk approve** | `bulkApproveAsDrafted()` → `syncBatchToCompendium()` | Batch POST (≤50 per request) for all approved codes |
| **Published page retry** | `PublishedResourcesTable` → `syncToCompendium()` / `syncBatchToCompendium()` | Manual retry for rows where `compendium_synced_at` is absent or `compendium_sync_error` is set |
