...
Full canonical taxonomies live on the production Compendium site. **Runtime source of truth for the curation pipeline:** `data/taxonomy/live_methodologies.json` (platform codes), `data/taxonomy/live_specialties.json` (specialty slugs), `data/taxonomy/live_subtypes.json` (resource subtypes grouped by type), and `data/taxonomy/live_skills.json` (foundation skills FS-01..FS-16). Refresh all four via `python -m scripts.fetch_live_taxonomy` (reads sitemap.xml + page titles; skill names are canonical). Vertex AI Search still grounds on the four MVP methodology cards below; the classifier may assign any live platform code, subtype code, or foundation skill code.
